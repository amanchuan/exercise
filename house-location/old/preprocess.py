...
def PreProccess():
	...
	"""
	shop_inf = LoadData('train-ccf_first_round_shop_info.csv')
	train_data = LoadData2('train-ccf_first_round_user_shop_behavior.csv')
	...
	#print(shop_inf.shape, train_data.shape)	# (8477, 6) (10000, 6)
	shop_mall_id = []	
	shop_int_number = []	
	...
	for i in range(num_train):
		shop_id = train_data[i,1]
		for j in range(num_shop):
			if shop_inf[j,0] == shop_id:
				shop_mall_id.append(shop_inf[j,-1])		# 在训练数据中追加商店所在的商场id
				shop_int_number.append(shop_seq[j])		# 追加商店序号id
	shop_mall_id = np.array(shop_mall_id).reshape(num_train, -1)
	...
	y_train = new_train_data[:,-1]	# (10000, 1)
	#x_train = np.delete(new_train_data, [1], axis=1)
	x_train = new_train_data[:,[3,4]].astype(np.float64)	# (10000, 2)
	#new_test_data = test_data[:, [4,5]].astype(np.float64)		# (10000, 2)
	print(x_train.shape)
	print(y_train.shape)
